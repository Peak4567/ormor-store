    ...
    Alpine.data(
        ...
        (initialProducts, defaultId, userRole = "member", currentUsername = "") => ({
            ...
            getFormattedTimes() {
                ...
                let slots =
                    this.selectedProduct.time_slots ||
                    this.selectedProduct.timeSlots;
                if (!slots) return [];
                ...
                slots.forEach((slot) => {
                    ...
                    if (!times.includes(endHr) && startHr !== endHr)
                        times.push(endHr);
                });
                return times.sort();
            },
            ...
            updateCalendar() {
                ...
                if (!this.selectedProduct) {
                    ...
                    this.products.forEach((p) => {
                        if (p.availableDatesArray) {
                            ...
                            if (
                                p.available_end &&
                                (!globalMax || p.available_end > globalMax)
                            )
                ...
                if (this.selectedProduct.availableDatesArray && this.selectedProduct.availableDatesArray.length > 0) {
                    ...
                    this.fpInstance.set(
                        "enable",
                        this.selectedProduct.availableDatesArray,
                    );
                } else if (this.selectedProduct.available_start) {
                    this.fpInstance.set("enable", [() => true]);
                    this.fpInstance.set(
                        "minDate",
                        this.selectedProduct.available_start,
                    );
                    this.fpInstance.set(
                        "maxDate",
            ...
            async confirmBooking() {
                if (
                    ...
                    showSwalAlert(
                        "warning",
                        "ข้อมูลไม่ครบถ้วน",
                        "กรุณากรอกข้อมูลให้ครบถ้วนก่อนยืนยันการจองครับ",
                    );
                    return;
                }
                const confirmResult = await Swal.fire({
                    ...
                    customClass: {
                        ...
                        cancelButton:
                            "rounded-xl px-6 py-2.5 font-bold text-gray-500",
                    },
                });
                ...
                try {
                    ...
                    showSwalAlert(
                        "error",
                        "เกิดข้อผิดพลาด",
                        error.message || "การเชื่อมต่อเซิร์ฟเวอร์ขัดข้อง",
                    );
                }
            },
        }),
